...
class Settings:
  ...
  memory_consistency_worker_enabled: bool = True
  memory_consistency_interval_seconds: int = 15
  memory_consistency_batch_size: int = 20
  memory_consistency_lock_timeout_seconds: int = 300
...
def load_settings(*, require_database: bool = True, env: dict[str, str] | None = None) -> Settings:
  ...
  return Settings(
    ...
    memory_consistency_worker_enabled=parse_bool(
      source.get("MEMORY_CONSISTENCY_WORKER_ENABLED"),
      fallback=True,
    ),
    memory_consistency_interval_seconds=parse_positive_int(
      source.get("MEMORY_CONSISTENCY_INTERVAL_SECONDS"),
      fallback=15,
    ),
    memory_consistency_batch_size=parse_positive_int(
      source.get("MEMORY_CONSISTENCY_BATCH_SIZE"),
      fallback=20,
    ),
    memory_consistency_lock_timeout_seconds=parse_positive_int(
      source.get("MEMORY_CONSISTENCY_LOCK_TIMEOUT_SECONDS"),
      fallback=300,
    ),
    app_root=app_root,
