...
def extract_reusable_lesson(
  *,
  ...
  if task_type == "full_generation":
    module_label = ", ".join(modules[:8]) if modules else "the requested modules"
    return (
      f"Avoid single static dashboard pages. Generate route-backed Auth, Onboarding, Dashboard, and "
      f"{module_label} experiences from one shared navigation/export contract; then build, test every route, "
      "and repair at most once before publishing the preview."
    )
