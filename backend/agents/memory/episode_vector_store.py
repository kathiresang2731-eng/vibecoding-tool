...
def episodic_vector_memory_fallback_enabled() -> bool:
  configured = os.getenv("ENABLE_EPISODIC_VECTOR_MEMORY_FALLBACK")
  if configured is None:
    return not _production_like_environment()
...
def episode_vector_health() -> dict[str, Any]:
  qdrant = qdrant_episode_search_enabled()
  fallback = episodic_vector_memory_fallback_enabled()
  enabled = episodic_vector_search_enabled()
  production = _production_like_environment()
  warnings: list[str] = []
  if enabled and fallback and not qdrant:
    warnings.append("episode vectors are using process-local memory fallback; configure WORKTUAL_QDRANT_URL for durable retrieval")
  if production and not qdrant:
    warnings.append("production-like environment requires durable episodic vector retrieval; configure WORKTUAL_QDRANT_URL")
  if production and fallback and not qdrant:
    warnings.append("production-like environment has non-durable episodic vector fallback enabled")
  return {
    "enabled": enabled,
    "engine": "qdrant" if qdrant else "memory" if fallback else "relational",
    "durable": qdrant,
    "production": production,
    "production_ready": (not production) or qdrant,
    "qdrant_configured": qdrant,
    "memory_fallback_enabled": fallback,
  ...
  def search(
    ...
    *,
    ...
    try:
      ...
      # Allows dependency-free unit tests with a mocked client. A real client
      # remains unavailable because _get_client() enforces qdrant-client.
      query_filter = {
        "must": [
          {"key": "user_id", "match": {"value": str(user_id)}},
          {"key": "project_id", "match": {"value": str(project_id)}},
          {"key": "chat_session_id", "match": {"value": str(chat_session_id)}},
        ]
      }
