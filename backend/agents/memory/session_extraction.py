...
def _session_already_extracted(store: Any, user: Any, project_id: str, chat_session_id: str) -> bool:
...
def _mark_session_extracted(store: Any, user: Any, *, project_id: str, chat_session_id: str) -> None:
  ...
  if hasattr(store, "get_memory_chat_session_state"):
    try:
      prior = store.get_memory_chat_session_state(
        user,
        project_id=project_id,
        chat_session_id=chat_session_id,
      )
    except TypeError:
      prior = store.get_memory_chat_session_state(user, chat_session_id=chat_session_id)
...
def extract_closed_session_memories(
  ...
  *,
  ...
  if _session_already_extracted(store, user, project_id, chat_session_id):
