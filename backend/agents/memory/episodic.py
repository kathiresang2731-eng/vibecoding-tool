  ...
  *,
  ...
  for item in memories:
    ...
    if stored_session_id != chat_session_id:
      continue
    if chat_topic_id:
      stored_topic_id = _text(metadata.get("chat_topic_id") or item.get("chat_topic_id"))
      if stored_topic_id != chat_topic_id:
        continue
...
def serialize_episodic_memory_for_api(row: dict[str, Any]) -> dict[str, Any]:
  ...
  return {
    ...
    "chat_topic_id": _text(metadata.get("chat_topic_id") or row.get("chat_topic_id")),
...
def episode_to_memory_row(episode: dict[str, Any]) -> dict[str, Any]:
  ...
  return {
    ...
    "metadata_json": {
      ...
      "chat_topic_id": episode.get("chat_topic_id") or metadata.get("chat_topic_id"),
...
def _list_structured_episodic_items(
  ...
  *,
  ...
  try:
    ...
    if "chat_topic_id" not in str(exc):
      raise
    episodes = store.list_memory_episodes(
      user,
      project_id=project_id,
...
def list_episodic_memories(
  ...
  *,
  ...
  if structured:
    filtered = filter_episodic_memories(structured, chat_session_id=chat_session_id, chat_topic_id=chat_topic_id)
  ...
  if not legacy_items:
    return []
  filtered = filter_episodic_memories(legacy_items, chat_session_id=chat_session_id, chat_topic_id=chat_topic_id)
  ranked = _rank_episodes_for_prompt(
    filtered,
    prompt=prompt,
    user=user,
...
def build_episodic_context_block(
  memories: list[dict[str, Any]],
  *,
  prompt: str = "",
  ...
  ranked = (
    memories
    if already_ranked or not _text(prompt)
    else rank_episodic_memories(memories, prompt=prompt)
  )
