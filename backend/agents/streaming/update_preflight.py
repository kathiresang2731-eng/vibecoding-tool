      ...
      memories.append(
        {
          ...
          "metadata_json": {
            "source": "preflight_session_memory",
            "chat_session_id": chat_session_id,
            "chat_topic_id": chat_topic_id,
          },
        }
  ...
  *,
  ...
  for raw_task in list(update_analysis.get("scoped_update_tasks") or []):
    ...
    if raw_task.get("group_paths") and paths:
      grouped_paths = [path for path in paths if path not in seen_paths]
      if str(raw_task.get("kind") or "") == "visual_style_group" and len(grouped_paths) > 5:
        grouped_chunks = [grouped_paths[index : index + 5] for index in range(0, len(grouped_paths), 5)][:3]
      else:
        grouped_chunks = [grouped_paths[:max_tasks]]
      ...
      for chunk_index, grouped_chunk in enumerate(grouped_chunks, start=1):
        if not grouped_chunk:
          continue
        for path in grouped_chunk:
          seen_paths.add(path)
        tasks.append(
          {
            "id": f"analysis-group-{chunk_index}-{'-'.join(path.replace('/', '-').lower() for path in grouped_chunk)[:100]}",
            "kind": str(raw_task.get("kind") or "file_group"),
            "summary": summary[:240],
            "candidate_files": grouped_chunk,
            "paths": grouped_chunk,
            "scope": scope,
            "depends_on": [],
          }
...
def format_update_analysis_worker_block(
  ...
  *,
  ...
  if interaction:
    parts = [
      f"component={interaction.get('component')}" if interaction.get("component") else "",
      f"trigger={interaction.get('trigger')}" if interaction.get("trigger") else "",
      f"expected={interaction.get('expected')}" if interaction.get("expected") else "",
      f"source={interaction.get('source_page')}" if interaction.get("source_page") else "",
      f"target={interaction.get('target_page_or_route')}" if interaction.get("target_page_or_route") else "",
    ]
    joined = "; ".join(part for part in parts if part)
    if joined:
      lines.append(f"- Interaction contract: {joined[:600]}")
  matched_ui = [
    item
    for item in list(update_analysis.get("matched_ui_elements") or [])
    if isinstance(item, dict)
  ]
  if matched_ui:
    assigned_paths = set((task or {}).get("paths") or []) if isinstance(task, dict) else set()
    scoped_matches = [
      item
      for item in matched_ui
      if not assigned_paths or str(item.get("path") or "") in assigned_paths
    ]
    if not scoped_matches:
      scoped_matches = matched_ui
    lines.append(
      f"- Related rendered UI matches: {int(update_analysis.get('project_ui_match_count') or len(matched_ui))} "
      f"match(es) across {len(set(str(item.get('path') or '') for item in matched_ui if item.get('path')))} file(s)."
    )
    for item in scoped_matches[:8]:
      lines.append(
        "  • "
        f"{item.get('path')}:{item.get('line') or '?'} "
        f"{item.get('element_kind') or 'element'} "
        f"text={str(item.get('text') or '')[:120]!r} "
        f"purpose={str(item.get('purpose') or '')[:160]!r}"
      )
  snippets = [
    item
    for item in list(update_analysis.get("scope_enrichment_snippets") or [])
    if isinstance(item, dict)
  ]
  if snippets:
    assigned_paths = set((task or {}).get("paths") or []) if isinstance(task, dict) else set()
    ...
    if scoped_snippets:
      lines.append("- Preloaded source anchors:")
      for item in scoped_snippets:
        snippet = re.sub(r"\s+", " ", str(item.get("snippet") or "")).strip()
        lines.append(f"  • {item.get('path')} [{item.get('kind') or 'context'}]: {snippet[:300]}")
...
def plan_file_work_with_update_preflight(
  ...
  *,
  ...
  if (
    ...
    and analysis_tasks
    and str(work_plan.get("planning_source") or "") != "llm_file_work_planner"
  ):
