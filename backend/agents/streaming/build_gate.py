...
def run_post_update_build_gate(
  *,
  ...
  preview_result = _run_staged_build(tool_context, user, project_id, files)
  ...
  while build_status != "ready" and build_attempts < _max_build_attempts():
    ...
    if strategy and repair_paths and strategy not in applied_deterministic:
      ...
      if build_attempts >= _max_build_attempts():
        break
      build_attempts += 1
      preview_result = _run_staged_build(tool_context, user, project_id, files)
      version = preview_result.get("version") if isinstance(preview_result.get("version"), dict) else {}
    ...
    emit_progress(
      "gate.repair.started",
      f"Repair agent fixing {', '.join(error_paths)} (attempt {repair_attempts}/{_max_repair_attempts()})",
      status="running",
  ...
  if build_status == "ready":
    ...
    emit_progress(
      "gate.build.failed",
      f"Preview build failed after {repair_attempts} repair attempt(s): {final_reason[:240]}",
      status="failed",
