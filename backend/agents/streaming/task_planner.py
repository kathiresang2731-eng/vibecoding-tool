...
def _is_non_page_label(label: str) -> bool:
  lowered = str(label or "").strip().lower()
  if not lowered:
    return True
  if any(marker in lowered for marker in _NON_PAGE_LABEL_MARKERS):
    return True
  module_markers = (
    "dashboard",
    "contact",
...
def plan_greenfield_parallel_tasks(prompt: str, *, max_tasks: int = 16) -> dict[str, Any]:
  """Multi-wave parallel plan for empty/incomplete projects.

  Each page/component/data file gets its own worker in wave 1 so files are generated concurrently.
  Platform-owned Vite scaffold files are injected before workers run; App.jsx wiring runs in wave 2.
  """
  try:
  ...
  task_budget = max(4, min(max_tasks, configured_task_cap))
  ...
  page_budget = max(3, task_budget - len(comp_paths) - len(data_paths) - 1)
  page_paths = _infer_greenfield_page_paths(prompt, max_pages=page_budget)
  backend_paths = _infer_greenfield_backend_paths(prompt)

  module_tasks: list[dict[str, Any]] = [
    *[_greenfield_page_task(path) for path in page_paths],
    *[_greenfield_component_task(path) for path in comp_paths],
    *[_greenfield_data_file_task(path) for path in data_paths],
    *[_greenfield_backend_task(path) for path in backend_paths],
  ]
  if len(module_tasks) > task_budget - 1:
    module_tasks = module_tasks[: task_budget - 1]
  for path in backend_paths:
    if not any(path in (task.get("paths") or []) for task in module_tasks):
      module_tasks.append(_greenfield_backend_task(path))

  shell_depends_on = [
    task["id"]
    for task in module_tasks
    if task.get("kind") in {"greenfield_page", "greenfield_component", "greenfield_pages", "greenfield_components"}
  ]
  app_task: dict[str, Any] = {
    "id": "greenfield-app-shell",
    "kind": "greenfield_app_shell",
    "paths": ["src/App.jsx"],
    "contract": {
      "role": "main_coding_agent_integration",
      "export_type": "default",
      "export_name": "App",
      "provides": ["src/App.jsx"],
      "imports_allowed": [_relative_import_from_app(path) for path in page_paths + comp_paths],
      "acceptance": "Imports only completed co-worker exports, defines routes/navigation, and keeps scaffold files untouched.",
    },
    "scope": (
      "Wire src/App.jsx as a thin React Router shell importing the generated pages and layout components. "
      "Import only completed co-worker exports from shared memory. "
      "Do not edit index.html, package.json, package-lock.json, vite.config.js, tailwind.config.js, "
      "src/index.css, or other platform scaffold files."
    ),
    "depends_on": shell_depends_on,
  }

  tasks = [*module_tasks, app_task]
  wave1_ids = [task["id"] for task in module_tasks]
  waves = [wave1_ids, ["greenfield-app-shell"]] if wave1_ids else [["greenfield-app-shell"]]
  waves = _resolve_wave_path_overlaps(waves, {task["id"]: task for task in tasks})
  ...
  return {
    ...
    "planning_source": "greenfield_parallel_planner",
    "website_type": website_type,
    "coordination_contract": _greenfield_coordination_contract(
      website_type=website_type,
      module_tasks=module_tasks,
      app_task=app_task,
    ),
    "scoped_targets": page_paths + comp_paths,
