  ...
  *,
  ...
  if task_contracts:
    ...
    for item in task_contracts[:16]:
      ...
      lines.append(
        f"- {item.get('task_id')}: {item.get('kind')} -> {paths}; export {export_name}"
        f"{f'; app import {import_path}' if import_path else ''}"
      )
...
def run_parallel_file_workers(
  *,
  ...
  if not work_plan:
    ...
    work_plan = plan_file_work(prompt, intent=intent, project_files=project_files)
  ...
  for wave_index, wave in enumerate(waves, start=1):
    ...
    if greenfield and intent == "website_generation":
      wave_tasks = [
        task for task in wave_tasks if str(task.get("kind") or "") != "greenfield_app_shell"
      ]
  ...
  if greenfield and intent == "website_generation":
    try:
      from ..generation_engine.app_shell import apply_deterministic_app_shell
    except ImportError:
      from backend.agents.generation_engine.app_shell import apply_deterministic_app_shell
    repaired_map, repaired = apply_deterministic_app_shell(staged, work_plan=work_plan)
    if repaired:
      staged = repaired_map
      shared_memory.update_file("src/App.jsx", repaired_map.get("src/App.jsx", ""))
      emit(
        "app.shell.synthesized",
        "Generated deterministic App.jsx routes from completed page modules",
        status="completed",
  ...
  if write_payload:
    ...
    if syntax_issues:
      ...
      write_payload = []
    if write_payload and hasattr(tool_context.store, "create_version"):
      raise_if_project_cancelled(project_id)
      try:
        try:
  ...
  try:
    from ..generation_engine.scaffold_persist import (
      ensure_visible_scaffold_in_store,
      visible_project_files_from_store,
    )
  except ImportError:
    from backend.agents.generation_engine.scaffold_persist import (
      ensure_visible_scaffold_in_store,
      visible_project_files_from_store,
    )
  ...
  generated_website = _build_generated_website(
    visible_payload or write_payload or [{"path": p, "content": staged[p]} for p in changed_paths],
  ...
  artifact_response = {
    "summary": combined_summary,
    "files": visible_payload or write_payload,
    "changed_paths": [item["path"] for item in (visible_payload or write_payload)],
    "changed_file_paths": [item["path"] for item in (visible_payload or write_payload)],
  }
  if update_validation:
