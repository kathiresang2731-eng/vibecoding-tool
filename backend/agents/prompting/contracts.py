...
Return exactly this compact JSON shape. Do not add extra top-level keys:
{
  "generated_website": {
    ...
    "headline": "short artifact headline",
    "subheadline": "one sentence summary",
    "files": [
      {
        "path": "safe document path ending in .md, .txt, .csv, or .pdf, for example README.md, docs/research-plan.md, reports/audit.csv, notes/summary.txt, or reports/history.pdf",
        "purpose": "short purpose string",
        "code": "complete document content"
      }
    ]
  },
  "implementation_notes": {
    "recommended_next_actions": ["short next action"],
    "self_checks": ["short check that the file format and content match the request"]
  }
}
"""
...
extra keys:
{
  "type": "greeting|question|general_query|web_search|needs_more_detail|project_info",
  "message": "string",
...
keys:
{
  "intent": "greeting|question|general_query|web_search|needs_more_detail|project_info|simple_code|document_artifact|website_generation|website_update",
  "next_action": "respond_and_collect_website_brief|answer_question|answer_general_query|search_web|request_website_details|summarize_current_project|write_standalone_code_file|write_document_artifact|generate_website|update_website",
  "next_tool": "handle_greeting|answer_question|answer_general_query|search_web|request_website_details|summarize_current_project|generate_simple_code_file|generate_document_artifact|analyze_prompt|analyze_update_request",
  "reason": "string under 220 characters — no quotes, newlines, or trailing punctuation outside JSON",
  "missing_fields": ["LLM-inferred missing detail; empty unless intent is needs_more_detail"],
  "clarification_question": "LLM-authored question; empty unless intent is needs_more_detail"
}
"""
...
Return exactly this JSON shape for update analysis. Do not add extra keys:
{
  ...
  "interaction": {
    ...
    "source_page": "page or component where the interaction starts, empty if unknown",
    "target_page_or_route": "expected destination page, route, state, or module, empty if not navigation-related",
    "confidence": 0.0
  },
  ...
  "targeted_patch": {
    ...
    "colors": ["optional Tailwind color family names only when the user explicitly names them"],
    "primary_hex": "optional explicit or LLM-selected hex color; required with all theme hex fields when returning an artifact theme",
    "secondary_hex": "optional explicit or LLM-selected hex color; required with all theme hex fields when returning an artifact theme",
    "accent_hex": "optional explicit or LLM-selected hex color; required with all theme hex fields when returning an artifact theme",
    "background_hex": "optional explicit or LLM-selected hex color; required with all theme hex fields when returning an artifact theme",
    "text_hex": "optional explicit or LLM-selected hex color; required with all theme hex fields when returning an artifact theme",
