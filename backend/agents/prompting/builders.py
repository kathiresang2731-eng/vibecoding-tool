...
- Select execution_strategy=deterministic_patch only when request_kind is
  brand_name_update, document_title_update, cta_text_update,
  theme_color_update, or pagination_page_size_update AND targeted_patch includes
  the concrete value needed to apply the update safely.
- Select execution_strategy=scoped_model_patch for bug_fix, feature_patch, and
  other bounded existing-project changes.
...
- For theme updates, targeted_patch.colors must contain the requested color
  names and/or the hex fields must contain the requested palette.
...
- needs_more_detail: broad new build request with no concrete domain/features.
- website_generation: concrete new website/app/backend/API/database project.
- project_info: inspect, summarize, explain, audit, status, or error-understanding
  questions only. This is conversation-only with no confirmation and no file writes.
  Examples: "give me the information about this website" => project_info;
  "understand this error" => project_info; "what changed in the previous update?" => project_info.
- simple_code: standalone code/program/function/script/snippet, not a website/app
  or current-project change. Examples: "write a code for reverse number in python" => simple_code;
  "generate a Java program for palindrome string" => simple_code.
- website_update: modify the current project, fix errors, apply a previous enhancement idea,
  or add/remove/configure features, backend route, PostgreSQL connection, API, DB model,
  schema, migration, seed file, page, navbar, footer, auth, checkout, or theme.
  Examples: "implement those enhancement ideas" => website_update;
...
Use website_update only when the user asks to fix, resolve, patch, update, change,
implement, or make code changes. Use project_info when the user only asks to understand, explain, analyze, or summarize.
...
- intent: "greeting", "needs_more_detail", "project_info", "simple_code", "website_generation", or "website_update"
