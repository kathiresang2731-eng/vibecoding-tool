...
from .constants import ALLOWED_EXACT_PATHS, ALLOWED_PREFIXES, ROOT_STANDALONE_CODE_EXTENSIONS
...
def strip_accidental_project_folder(path: str) -> str:
  parts = path.split("/")
  if len(parts) >= 2 and parts[1] in PROJECT_ROOT_PREFIXES and parts[0] not in PROJECT_ROOT_PREFIXES and parts[0] not in ALLOWED_EXACT_PATHS:
    return "/".join(parts[1:])
  return path
...
def normalize_artifact_path(path: str) -> str:
  ...
  if is_allowed_worktual_meta_path(cleaned):
    return cleaned
  if "/" not in cleaned and cleaned.endswith(ROOT_STANDALONE_CODE_EXTENSIONS) and ROOT_STANDALONE_CODE_FILENAME_RE.match(cleaned):
    return cleaned
