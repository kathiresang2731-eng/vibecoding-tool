...
BRAND_COLOR_FAMILIES = "blue|indigo|violet|purple|fuchsia|pink|cyan|sky|teal|emerald"
NEUTRAL_COLOR_FAMILIES = "slate|gray|zinc|neutral|stone"
TAILWIND_BRAND_COLOR_CLASS_RE = re.compile(
  rf"(?P<prefix>(?:hover:|focus:|active:|group-hover:|focus-visible:)?(?:bg|text|border|ring|from|via|to|outline|decoration|accent|caret))-(?P<family>{BRAND_COLOR_FAMILIES})-(?P<shade>\d{{2,3}})"
)
TAILWIND_NEUTRAL_BACKGROUND_RE = re.compile(
  rf"(?P<prefix>(?:bg|from|via|to))-(?P<family>{NEUTRAL_COLOR_FAMILIES})-(?P<shade>50|100|200|900|950)"
)
...
def apply_targeted_file_update(files: list[dict[str, str]], request: dict[str, Any]) -> list[dict[str, str]]:
  ...
  if kind == "theme_color_update":
    return apply_theme_color_update(files, request)
...
def apply_theme_color_update(files: list[dict[str, str]], request: dict[str, Any]) -> list[dict[str, str]]:
  changed: list[dict[str, str]] = []
  for file_item in files:
    path = file_item["path"]
    content = file_item["content"]
    if not path.endswith((".css", ".js", ".jsx", ".ts", ".tsx")):
      continue
    updated = apply_theme_palette_to_content(path, content, request)
    if updated != content:
      changed.append({"path": path, "content": updated})
  return changed


def apply_theme_palette_to_content(path: str, content: str, request: dict[str, Any]) -> str:
  primary = text_or_default(request.get("primary"), "red")
  secondary = text_or_default(request.get("secondary"), primary)
  if primary not in COLOR_PALETTE_HEX or secondary not in COLOR_PALETTE_HEX:
    return ensure_theme_css_variables(content, request) if path.endswith(".css") else content

  def brand_replacer(match: re.Match[str]) -> str:
    prefix = match.group("prefix")
    shade = match.group("shade")
    family = secondary if prefix in {"to", "via"} else primary
    if prefix.endswith(":bg") and shade in {"50", "100", "200"}:
      family = secondary
    return f"{prefix}-{family}-{normalize_theme_shade(prefix, shade, family)}"

  def neutral_background_replacer(match: re.Match[str]) -> str:
    prefix = match.group("prefix")
    shade = match.group("shade")
    if prefix == "from" and shade in {"900", "950"}:
      return f"{prefix}-{primary}-950"
    if prefix in {"to", "via"}:
      return f"{prefix}-{secondary}-100"
    return f"{prefix}-{secondary}-50"

  updated = TAILWIND_BRAND_COLOR_CLASS_RE.sub(brand_replacer, content)
  updated = TAILWIND_NEUTRAL_BACKGROUND_RE.sub(neutral_background_replacer, updated)
  if path.endswith(".css"):
    updated = ensure_theme_css_variables(updated, request)
  return updated


def normalize_theme_shade(prefix: str, shade: str, family: str) -> str:
  try:
    value = int(shade)
  except ValueError:
    value = 600
  if family == "yellow":
    if prefix in {"text", "border", "ring", "outline", "decoration", "accent", "caret"}:
      return "600" if value >= 500 else "500"
    if value >= 700:
      return "500"
    if value >= 500:
      return "400"
    return str(max(value, 100))
  if value <= 100:
    return str(value)
  return str(min(max(value, 500), 700))


def ensure_theme_css_variables(content: str, request: dict[str, Any]) -> str:
  block = (
    ":root {\n"
    f"  --vibe-theme-primary: {request['primary_hex']};\n"
    f"  --vibe-theme-secondary: {request['secondary_hex']};\n"
    f"  --vibe-theme-background: {request['background_hex']};\n"
    "}\n"
  )
  if "--vibe-theme-primary" in content:
    content = re.sub(r":root\s*\{[^{}]*--vibe-theme-primary:[^{}]*\}\s*", block, content, count=1, flags=re.DOTALL)
  else:
    content = f"{block}\n{content}"
  def body_background_replacer(match: re.Match[str]) -> str:
    body_block = match.group(0)
    if re.search(r"background(?:-color)?\s*:", body_block):
      return re.sub(
        r"background(?:-color)?\s*:[^;}\n]+;?",
        "background: var(--vibe-theme-background);",
        body_block,
        count=1,
      )
    return re.sub(r"body\s*\{", "body {\n  background: var(--vibe-theme-background);\n", body_block, count=1)

  content, replacement_count = re.subn(r"body\s*\{[^{}]*\}", body_background_replacer, content, count=1, flags=re.DOTALL)
