...
def normalize_update_analysis(
  ...
  *,
  ...
  return {
    ...
    "feature_plan": feature_plan,
    "candidate_files": candidate_files,
    "target_files": string_list(raw.get("target_files"), [])[:4],
...
def normalize_targeted_patch_intent(value: Any, *, request_kind: str) -> dict[str, Any]:
  ...
  for raw_color in string_list(raw.get("colors"), []):
    ...
    if color in COLOR_PALETTE_HEX and color not in colors:
      colors.append(color)
    if len(colors) >= 4:
      break
  return {
...
def targeted_patch_is_actionable(targeted_patch: dict[str, Any], request_kind: str) -> bool:
  ...
  if kind == "theme_color_update":
    return bool(targeted_patch.get("colors") or targeted_patch.get("primary_hex") or targeted_patch.get("secondary_hex"))
  return False
...
def targeted_update_request_from_analysis(update_analysis: dict[str, Any]) -> dict[str, Any] | None:
  ...
  if kind == "theme_color_update":
    colors = [color for color in string_list(targeted_patch.get("colors"), []) if color in COLOR_PALETTE_HEX]
    ...
    if colors:
      ...
      return {
        ...
        "primary": primary,
        "secondary": secondary,
        "primary_hex": COLOR_PALETTE_HEX[primary],
        "secondary_hex": COLOR_PALETTE_HEX[secondary],
        "background_hex": COLOR_BACKGROUND_HEX.get(secondary, COLOR_BACKGROUND_HEX.get(primary, "#fff7ed")),
    ...
    if primary_hex or secondary_hex:
      return {
        ...
        "primary": "custom",
        "secondary": "custom",
        "primary_hex": primary_hex or secondary_hex,
        "secondary_hex": secondary_hex or primary_hex,
        "background_hex": background_hex or "#ffffff",
