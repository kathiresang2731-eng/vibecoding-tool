...
def apply_targeted_update_shortcut(state: dict[str, Any], *, project_id: str) -> bool:
  ...
  theme_colors = {
    "primary": str(request.get("primary_hex") or "").strip(),
    "secondary": str(request.get("secondary_hex") or "").strip(),
    "accent": str(request.get("accent_hex") or "").strip(),
    "background": str(request.get("background_hex") or "").strip(),
    "text": str(request.get("text_hex") or "").strip(),
  }
  has_complete_theme = all(theme_colors.values())
  generated_website = {
    "title": title,
  ...
  if has_complete_theme:
    generated_website["theme"] = {
      "colors": theme_colors,
  ...
  state["brief"] = {
    ...
    "style": f"Targeted {update_label}" if has_complete_theme else "Preserve existing website visual design.",
