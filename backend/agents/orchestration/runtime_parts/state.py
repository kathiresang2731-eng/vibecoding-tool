...
def existing_agentic_runtime(response: dict[str, Any]) -> dict[str, Any] | None:
  ...
  if not isinstance(runtime, dict):
    return None
  if runtime.get("tool_source_of_truth") and (
    isinstance(runtime.get("steps"), list)
    or isinstance(runtime.get("tool_calls"), list)
  ):
    return runtime
  if str(runtime.get("status") or "").lower() == "failed" or bool(runtime.get("no_code_changes")):
    return runtime
  return None
...
def apply_backend_routing_to_response(state: GenerationPipelineState) -> None:
  ...
  default_sequence = (
    ...
    if state.intent == "simple_code"
    else ["route_generation_action", "generate_document_artifact", "validate_generated_website"]
    if state.intent == "document_artifact"
    else [
