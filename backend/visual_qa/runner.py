  ...
  with tempfile.TemporaryDirectory(prefix="worktual-preview-qa-", ignore_cleanup_errors=True) as temp_dir:
    ...
    try:
      ...
      layout_result = {
        ...
        "viewport_results": skipped_layout_viewport_results(
          f"Browser layout probe unavailable: {exc}"
        ),
        "layout_issues": [],
        "severity": "none",
        "warnings": [f"Browser layout probe unavailable: {exc}"],
      }
    ...
    if prompt_requires_interaction_qa(interaction_prompt):
      interaction_result = run_browser_interaction_qa(
        browser_command=browser_command,
        target_url=target_url,
        profile_path=Path(temp_dir) / "interaction",
        prompt=interaction_prompt,
      )
      interaction_passed = interaction_result.get("status") == "passed"
      checks.append(
        {
          "name": "requested_control_interaction",
          "status": "passed" if interaction_passed else "failed",
          "detail": (
            f"Clicked the requested control; final URL {interaction_result.get('current_url')}"
            if interaction_passed
            else str(interaction_result.get("reason") or "Interaction QA failed.")
          ),
        }
      )
      if not interaction_passed:
        warnings.append(str(interaction_result.get("reason") or "Requested interaction failed."))
    if layout_checked:
    ...
    return {
      ...
      "status": (
        "failed"
        if severity == "high" or (interaction_result and interaction_result.get("status") != "passed")
        else "passed"
      ),
      "mode": "browser_rendered_preview",
      "failure_kind": (
        str(interaction_result.get("failure_kind") or "")
        if interaction_result and interaction_result.get("status") != "passed"
        else ""
      ),
