(function () {
  var base = "__BASE_HREF__".replace(/\/+$/, "");
  ...
  try {
    ...
    localStorage.setItem("worktual_active_preview_project", "__PROJECT_ID__");
    localStorage.setItem("worktual_active_preview_version", "__VERSION_ID__");
  ...
  function syncPreviewRouteMeta() {
    try {
      ...
      document.documentElement.setAttribute("data-worktual-preview-project", "__PROJECT_ID__");
      document.documentElement.setAttribute("data-worktual-preview-version", "__VERSION_ID__");
    } catch (err) {
      /* ignore */
    }
  }
