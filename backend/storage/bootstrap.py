...
BOOTSTRAP_STATEMENTS = [
      ...
      """,
      "alter table project_files add column if not exists content_hash text not null default ''",
      """
      create table if not exists project_code_index_chunks (
        project_id text not null references projects(id) on delete cascade,
        path text not null,
        ...
        created_at timestamptz not null default now(),
        updated_at timestamptz not null default now(),
        primary key (project_id, path, chunk_id)
      )
      """,
      "alter table project_code_index_chunks add column if not exists embedding_model text not null default ''",
      "alter table project_code_index_chunks add column if not exists embedding_dimensions integer not null default 0",
      """
      ...
      """,
      """
      create table if not exists memory_chat_topics (
        id text primary key,
        project_id text not null references projects(id) on delete cascade,
        user_id text not null references users(id) on delete cascade,
        ...
        memory_scope text not null default 'topic',
        topic_tags text not null default '',
        rolling_summary text not null default '',
        related_paths_json jsonb not null default '[]'::jsonb,
        related_modules_json jsonb not null default '[]'::jsonb,
      ...
      """,
      "alter table project_chat_messages add column if not exists chat_topic_id text references memory_chat_topics(id) on delete set null",
      "create unique index if not exists idx_chat_sessions_scope_unique on project_chat_sessions(id, project_id, user_id)",
      "create unique index if not exists idx_chat_topics_scope_unique on memory_chat_topics(id, project_id, user_id, chat_session_id)",
      "create unique index if not exists idx_generation_runs_scope_unique on generation_runs(id, project_id, user_id)",
      """
      do $$
      begin
        if not exists (select 1 from pg_constraint where conname = 'fk_chat_topics_session_scope') then
          alter table memory_chat_topics
            add constraint fk_chat_topics_session_scope
            foreign key (chat_session_id, project_id, user_id)
            references project_chat_sessions(id, project_id, user_id)
            on delete cascade not valid;
        end if;
        if not exists (select 1 from pg_constraint where conname = 'fk_chat_messages_session_scope') then
          alter table project_chat_messages
            add constraint fk_chat_messages_session_scope
            foreign key (chat_session_id, project_id, user_id)
            references project_chat_sessions(id, project_id, user_id)
            on delete cascade not valid;
        end if;
        if not exists (select 1 from pg_constraint where conname = 'fk_chat_messages_topic_scope') then
          alter table project_chat_messages
            add constraint fk_chat_messages_topic_scope
            foreign key (chat_topic_id, project_id, user_id, chat_session_id)
            references memory_chat_topics(id, project_id, user_id, chat_session_id)
      ...
      "create index if not exists idx_project_code_index_project_path on project_code_index_chunks(project_id, path)",
      "create index if not exists idx_project_code_index_file_hash on project_code_index_chunks(project_id, file_content_hash)",
      ...
      create table if not exists memory_episodes (
        ...
        project_id text not null references projects(id) on delete cascade,
        chat_session_id text references project_chat_sessions(id) on delete set null,
        chat_topic_id text references memory_chat_topics(id) on delete set null,
        generation_run_id text references generation_runs(id) on delete set null,
      ...
      """,
      "alter table memory_episodes add column if not exists vector_status text not null default 'pending'",
      "alter table memory_episodes add column if not exists vector_error text not null default ''",
      "alter table memory_episodes add column if not exists vector_updated_at timestamptz",
      """
      ...
      create table if not exists memory_chat_session_state (
        ...
        project_id text not null references projects(id) on delete cascade,
        user_id text not null references users(id) on delete cascade,
        state_scope_key text not null default '',
        chat_topic_id text references memory_chat_topics(id) on delete set null,
        rolling_summary text not null default '',
        last_changed_paths_json jsonb not null default '[]'::jsonb,
      ...
      """,
      "alter table memory_chat_session_state add column if not exists state_scope_key text not null default ''",
      "alter table memory_chat_session_state add column if not exists chat_topic_id text references memory_chat_topics(id) on delete set null",
      """
      do $$
      begin
        if exists (
          select 1
          from pg_constraint
          where conname = 'memory_chat_session_state_pkey'
            and conrelid = 'memory_chat_session_state'::regclass
        ) then
          alter table memory_chat_session_state drop constraint memory_chat_session_state_pkey;
        end if;
        if not exists (
          select 1
          from pg_constraint
          where conname = 'memory_chat_session_state_scope_pkey'
            and conrelid = 'memory_chat_session_state'::regclass
        ) then
          alter table memory_chat_session_state
            add constraint memory_chat_session_state_scope_pkey primary key (chat_session_id, state_scope_key);
        end if;
      end $$;
      """,
      """
      ...
      """,
      "alter table memory_episodes add column if not exists chat_topic_id text references memory_chat_topics(id) on delete set null",
      "alter table memory_session_snapshots add column if not exists chat_topic_id text references memory_chat_topics(id) on delete set null",
      "alter table memory_learning_events add column if not exists chat_topic_id text references memory_chat_topics(id) on delete set null",
      """
      do $$
      begin
        if not exists (select 1 from pg_constraint where conname = 'fk_memory_episodes_session_scope') then
          alter table memory_episodes
            add constraint fk_memory_episodes_session_scope
            foreign key (chat_session_id, project_id, user_id)
            references project_chat_sessions(id, project_id, user_id)
            on delete no action not valid;
        end if;
        if not exists (select 1 from pg_constraint where conname = 'fk_memory_episodes_topic_scope') then
          alter table memory_episodes
            add constraint fk_memory_episodes_topic_scope
            foreign key (chat_topic_id, project_id, user_id, chat_session_id)
            references memory_chat_topics(id, project_id, user_id, chat_session_id)
            on delete no action not valid;
        end if;
        if not exists (select 1 from pg_constraint where conname = 'fk_memory_episodes_run_scope') then
          alter table memory_episodes
            add constraint fk_memory_episodes_run_scope
            foreign key (generation_run_id, project_id, user_id)
            references generation_runs(id, project_id, user_id)
            on delete no action not valid;
        end if;
        if not exists (select 1 from pg_constraint where conname = 'fk_memory_snapshots_session_scope') then
          alter table memory_session_snapshots
            add constraint fk_memory_snapshots_session_scope
            foreign key (chat_session_id, project_id, user_id)
            references project_chat_sessions(id, project_id, user_id)
            on delete cascade not valid;
        end if;
        if not exists (select 1 from pg_constraint where conname = 'fk_memory_snapshots_topic_scope') then
          alter table memory_session_snapshots
            add constraint fk_memory_snapshots_topic_scope
            foreign key (chat_topic_id, project_id, user_id, chat_session_id)
            references memory_chat_topics(id, project_id, user_id, chat_session_id)
            on delete no action not valid;
        end if;
        if not exists (select 1 from pg_constraint where conname = 'fk_memory_snapshots_run_scope') then
          alter table memory_session_snapshots
            add constraint fk_memory_snapshots_run_scope
            foreign key (generation_run_id, project_id, user_id)
            references generation_runs(id, project_id, user_id)
            on delete no action not valid;
        end if;
        if not exists (select 1 from pg_constraint where conname = 'fk_memory_state_session_scope') then
          alter table memory_chat_session_state
            add constraint fk_memory_state_session_scope
            foreign key (chat_session_id, project_id, user_id)
            references project_chat_sessions(id, project_id, user_id)
            on delete cascade not valid;
        end if;
        if not exists (select 1 from pg_constraint where conname = 'fk_memory_state_topic_scope') then
          alter table memory_chat_session_state
            add constraint fk_memory_state_topic_scope
            foreign key (chat_topic_id, project_id, user_id, chat_session_id)
            references memory_chat_topics(id, project_id, user_id, chat_session_id)
            on delete no action not valid;
        end if;
        if not exists (select 1 from pg_constraint where conname = 'fk_memory_state_run_scope') then
          alter table memory_chat_session_state
            add constraint fk_memory_state_run_scope
            foreign key (last_generation_run_id, project_id, user_id)
            references generation_runs(id, project_id, user_id)
            on delete no action not valid;
        end if;
      end $$;
      """,
      """
      create table if not exists project_consistency_jobs (
        id text primary key,
        project_id text not null references projects(id) on delete cascade,
        ...
        created_at timestamptz not null default now(),
        updated_at timestamptz not null default now(),
        unique (project_id, job_type, target_key, source_hash)
      )
      """,
      """
      create table if not exists memory_checkpoint_commits (
        generation_run_id text primary key references generation_runs(id) on delete cascade,
        project_id text not null references projects(id) on delete cascade,
        user_id text not null references users(id) on delete cascade,
        chat_session_id text not null references project_chat_sessions(id) on delete cascade,
        chat_topic_id text references memory_chat_topics(id) on delete set null,
      ...
      """,
      "create index if not exists idx_memory_chat_topics_session_updated on memory_chat_topics(project_id, chat_session_id, updated_at desc)",
      "create index if not exists idx_memory_chat_topics_intent on memory_chat_topics(chat_session_id, intent_family, updated_at desc)",
      "create index if not exists idx_chat_messages_topic_created on project_chat_messages(chat_topic_id, created_at)",
      ...
      "create index if not exists idx_memory_episodes_topic_created on memory_episodes(chat_topic_id, created_at desc)",
      ...
      "create index if not exists idx_memory_session_snapshots_topic on memory_session_snapshots(chat_topic_id, created_at desc)",
      "create index if not exists idx_memory_chat_session_state_topic on memory_chat_session_state(chat_topic_id, updated_at desc)",
      "create index if not exists idx_memory_learning_events_user_created on memory_learning_events(user_id, created_at desc)",
      "create index if not exists idx_memory_learning_events_topic on memory_learning_events(chat_topic_id, created_at desc)",
      ...
      "create index if not exists idx_consistency_jobs_pending on project_consistency_jobs(status, available_at, created_at) where status in ('pending', 'failed')",
      "create index if not exists idx_consistency_jobs_project on project_consistency_jobs(project_id, created_at desc)",
      "create index if not exists idx_memory_episodes_vector_status on memory_episodes(vector_status, updated_at desc)",
      "create index if not exists idx_memory_snapshots_run on memory_session_snapshots(project_id, chat_session_id, generation_run_id, snapshot_kind)",
      """
      create unique index if not exists idx_memory_snapshots_unique_run_kind
      on memory_session_snapshots(project_id, chat_session_id, generation_run_id, snapshot_kind)
      where generation_run_id is not null
      """,
      "create index if not exists idx_memory_checkpoint_commits_status on memory_checkpoint_commits(status, updated_at)",
      """
