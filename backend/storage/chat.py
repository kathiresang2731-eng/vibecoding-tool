...
class ChatHistoryStoreMixin:
  ...
  def record_project_chat_message(
    ...
    *,
    ...
    normalized_topic_id = str(chat_topic_id or metadata_payload.get("chat_topic_id") or "").strip() or None
    if normalized_topic_id:
      require_memory_scope(
        ...
        chat_topic_id=normalized_topic_id,
        write=True,
      )
    ...
    with self.connect() as conn:
      with conn.cursor() as cursor:
        cursor.execute(
          """
          insert into project_chat_messages (id, project_id, user_id, chat_session_id, chat_topic_id, role, content, metadata_json)
          values (%s, %s, %s, %s, %s, %s, %s, %s::jsonb)
          returning id, project_id, user_id, chat_session_id, chat_topic_id, role, content, metadata_json, created_at
          """,
          (
            message_id,
            project_id,
            ...
            normalized_role,
            content,
            json_dumps_safe(metadata_payload, context="chat.metadata"),
          ),
        )
        row = cursor.fetchone()
  ...
  def list_project_chat_messages(
    ...
    *,
    ...
    topic_filter = "and chat_topic_id = %s" if normalized_topic_id else ""
    params: tuple[Any, ...] = (
      project_id,
      user.id,
      resolved_session_id,
    ...
    with self.connect() as conn:
      with conn.cursor() as cursor:
        cursor.execute(
          f"""
          select id, project_id, user_id, chat_session_id, chat_topic_id, role, content, metadata_json, created_at
          from (
            select id, project_id, user_id, chat_session_id, chat_topic_id, role, content, metadata_json, created_at
