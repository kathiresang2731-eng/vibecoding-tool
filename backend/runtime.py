...
def prepare_workspace(app_root: Path, project_id: str) -> Path:
  runtime_root = runtime_projects_root(app_root) / project_id
...
def runtime_project_path(app_root: Path, project_id: str, version_id: str) -> Path:
  return runtime_projects_root(app_root) / project_id / version_id


def delete_project_runtime(app_root: Path, project_id: str) -> None:
  runtime_root = runtime_projects_root(app_root)
...
def runtime_projects_root(app_root: Path) -> Path:
  configured = str(os.getenv("WORKTUAL_RUNTIME_ROOT") or "").strip()
  if configured:
    root = Path(configured).expanduser()
    if not root.is_absolute():
      root = app_root / root
    root = root if root.name == "projects" else root / "projects"
  else:
    root_hash = hashlib.sha1(str(app_root.resolve(strict=False)).encode("utf-8")).hexdigest()[:12]
    root = Path(tempfile.gettempdir()) / f"worktual_runtime_{root_hash}" / "projects"
