...
| Show patch diff before commit | `src/main.jsx` + `patch.proposed` events | Partial (events emitted; UI next) |
| Scoped update → APPLY_PATCH | scoped update agents | Done |
| Gate failure messages in chat | `failures.py` → UI | User knows why build failed |

### Tier 3 — Rich context (Phase 2)

| Task | Outcome |
|------|---------|
| SEARCH_CODEBASE in context pack | @codebase-style relevance |
| AGENTS.md in project root | Per-project rules |
