...
def test_greenfield_generation_uses_single_streaming_agent_by_default(monkeypatch):
  ...
  def fake_streaming_file_agent(**kwargs):
    ...
    return {
      ...
      "artifact_response": {},
      "runtime": {"engine": "streaming_file_agent", "changed_paths": []},
    }
  ...
  monkeypatch.setattr(
    ...
    lambda **_kwargs: (_ for _ in ()).throw(AssertionError("parallel workers should not run")),
  )
  ...
  assert result["runtime"]["workflow"] == "greenfield_single_streaming_agent"
  assert result["runtime"]["parallel_workers_skipped"] is True
