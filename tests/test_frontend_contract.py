...
def test_generation_failure_chat_uses_backend_message_without_duplicate_prefix():
  source = Path("src/main.jsx").read_text(encoding="utf-8")

  assert "function formatAssistantGenerationFailure(message)" in source
  assert "content: formatAssistantGenerationFailure(nextError.message)" in source
  assert "content: `Generation failed: ${nextError.message}`" not in source


def test_auto_preview_placeholder_uses_current_prompt_not_only_project_name():
  source = Path("src/main.jsx").read_text(encoding="utf-8")
  preview_copy = source.split("function previewPlaceholderCopy", 1)[1].split("function openGenerationPreviewPlaceholder", 1)[0]

  assert "function previewPlaceholderCopy(projectName = \"\", prompt = \"\")" in source
  assert "function previewRequestTitleFromPrompt(prompt = \"\", projectName = \"\")" in source
  assert "openGenerationPreviewPlaceholder(project?.name || DEFAULT_PROJECT_NAME, effectivePrompt)" in source
  assert "<strong style=\"color:#334155\">Request:</strong> ${safeSummary}" in source
  assert "const titlePrefix = isUpdate ? \"Updating\" : \"Preparing\";" in source
  assert "const requestTitle = previewRequestTitleFromPrompt(rawPrompt, projectName);" in preview_copy
  assert "projectTitleFromPrompt(rawPrompt)" not in preview_copy
  assert "`Counting Buttons On ${pageLabel}`" in source
  assert '"Referenced Page"' in source
  assert "Replacing Popup With Modal" in source
...
def test_generation_records_browser_write_back_progress_after_backend_commit():
  ...
  assert "let filesToApply = persistedFiles.length ? persistedFiles : generatedFiles;" in source
  assert "applySyncedFiles(filesToApply, generatedFiles[0]?.path)" in source
...
def test_failed_update_runtime_does_not_write_staged_files_to_browser_workspace():
  source = Path("src/main.jsx").read_text(encoding="utf-8")
  realtime_sync_source = source.split("async function applyRealtimeGenerationProgress", 1)[1].split(
    "async function syncGeneratedFilesToBrowserWorkspace",
    1,
  )[0]
  final_payload_source = source.split("function previewVersionFromGenerationPayload", 1)[1].split(
    "function encodePath",
    1,
  )[0]

  assert 'const fileSteps = new Set(["files.persisted"]);' in realtime_sync_source
  assert "patch.proposed" not in realtime_sync_source
  assert "file.written" not in realtime_sync_source
  assert "files.materialized" not in realtime_sync_source
  assert "await writeProjectFilesToBrowserWorkspace(projectId, files);" in realtime_sync_source
  assert 'if (progressStep === "files.persisted") {' in source
  assert '["file.written", "files.materialized", "files.persisted", "patch.proposed"]' not in source
  assert 'if (String(runtime?.status || "").toLowerCase() === "failed") return null;' in final_payload_source
  assert 'if (String(runtime?.status || "").toLowerCase() === "failed") return [];' in final_payload_source
  assert "runtime?.commit_result && runtime.commit_result.persisted === false" in final_payload_source
  assert 'String(runtimePreview.status || "ready") === "ready"' in final_payload_source
  assert 'String(buildGate.status || "ready") === "ready"' in final_payload_source
