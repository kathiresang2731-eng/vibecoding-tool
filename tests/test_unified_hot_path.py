...
def test_runner_lazy_imports_parallel_orchestrator() -> None:
  ...
  runner_path = Path(__file__).resolve().parents[1] / "backend/agents/orchestration/runner_parts/core.py"
  website_runtime_path = Path(__file__).resolve().parents[1] / "backend/agents/orchestration/runner_parts/website_runtime.py"
  source = runner_path.read_text()
  website_runtime_source = website_runtime_path.read_text()
  header, _body = source.split("class ", 1)
  assert "parallel_orchestrator" not in header
  assert "from backend.agents.streaming.parallel_orchestrator import run_parallel_stream_orchestrator" in website_runtime_source
