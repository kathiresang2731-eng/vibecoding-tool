...
def test_greenfield_planner_uses_parallel_waves():
  ...
  assert plan.get("planning_source") == "greenfield_parallel_planner"
  assert plan.get("task_count", 0) >= 4
  assert plan.get("use_parallel_workers") is True
  assert plan.get("wave_count") == 2
  wave1 = plan["waves"][0]
  assert len(wave1) >= 3
  assert any(task_id.startswith("greenfield-page-") for task_id in wave1)
  assert any(task_id.startswith("greenfield-component-") for task_id in wave1)
  assert plan["waves"][1] == ["greenfield-app-shell"]
  task_ids = {task["id"] for task in plan["tasks"]}
  assert "greenfield-scaffold" not in task_ids
  assert any(task_id.startswith("greenfield-page-") for task_id in task_ids)
  assert "greenfield-app-shell" in task_ids
