...
def test_post_update_visual_qa_emits_advisory_event_with_structured_detail(monkeypatch) -> None:
  monkeypatch.setenv("ENABLE_POST_UPDATE_VISUAL_QA", "true")
  ...
  advisory = next(item for item in captured if item["step"] == "gate.visual_qa.advisory")
...
def test_post_update_visual_qa_timeout_is_inconclusive_not_failed(monkeypatch) -> None:
  monkeypatch.setenv("ENABLE_POST_UPDATE_VISUAL_QA", "true")
  captured: list[dict[str, object]] = []

  def fake_tool(_context, _user, _arguments):
    raise TimeoutError("Browser render timed out after 20s")
  ...
  assert result is not None
  assert result["status"] == "failed"
  assert result["code"] == "visual_qa_timeout"
  inconclusive = next(item for item in captured if item["step"] == "gate.visual_qa.inconclusive")
  assert inconclusive["status"] == "completed"
  detail = inconclusive["detail"]
  assert detail["code"] == "visual_qa_timeout"
  assert detail["files_committed"] is True
  assert detail["advisory"] is True
