...
class LLMContractTests(unittest.TestCase):
  ...
  def test_failed_update_runtime_is_preserved_during_response_normalization(self):
    from backend.agents.orchestration.runtime_parts.traces import attach_live_runtime_metadata

    state = GenerationPipelineState(
      user_prompt="change the website theme to red and black",
      intent="website_update",
      routing_result={"intent": "website_update", "next_tool": "generate_update_artifact", "reason": "update"},
    )
    state.response = {
      "multi_agent_system": {
        "agentic_runtime": {
          "runtime": "gemini-native-control-artifact",
          "workflow": "gemini-native-control-artifact",
          "status": "failed",
          "no_code_changes": True,
          "tool_source_of_truth": False,
          "output_text": "Scoped update produced zero changed files.",
          "changed_paths": [],
        }
      },
      "gemini_tool_calling_setup": {
    ...
    runtime = state.response["multi_agent_system"]["agentic_runtime"]
    self.assertEqual(runtime["status"], "failed")
    self.assertTrue(runtime["no_code_changes"])
    self.assertIn("zero changed files", runtime["output_text"])
    ...
    self.assertEqual(runtime["status"], "failed")
    self.assertTrue(runtime["no_code_changes"])
    self.assertIsInstance(runtime.get("steps"), list)
  ...
  def test_project_info_conversation_prompt_requests_summary_not_confirmation(self):
    ...
    self.assertIn("Do not wrap headings or labels in markdown stars", prompt)
  ...
  def test_artifact_path_validation_allows_generation_plan_documents(self):
    self.assertEqual(normalize_artifact_path("todo.md"), "todo.md")
    self.assertEqual(normalize_artifact_path("WEBSITE.md"), "WEBSITE.md")
  ...
  def test_greeting_prompt_uses_llm_tiny_chat_and_no_file_updates(self):
    class FakeStore:
      def __init__(self):
        self.chat_messages = []
        self.tool_calls = []
    ...
    self.assertEqual(result["generation_run"]["provider"], "gemini-native-tiny-chat")
    ...
    self.assertEqual(conversation["message"], "Hi. Tell me what you want to build or update next.")
