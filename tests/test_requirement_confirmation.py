...
def test_confirmed_brief_resumes_original_request(monkeypatch):
  ...
  monkeypatch.setattr(website_runtime_module, "execute_real_agent_runtime_loop", fake_runtime)
...
def test_confirmed_brief_reclassifies_original_request_for_greenfield_generation(monkeypatch):
  context, _ = confirmation_context()
  first_provider = ConfirmationProvider(route_intent="website_generation")
  ...
  def fake_greenfield_runtime(**kwargs):
    captured.update(kwargs)
    return runtime_result()

  monkeypatch.setattr(website_runtime_module, "run_website_generation", fake_greenfield_runtime)
