...
def test_plan_greenfield_ccaas_infers_pages_and_parallel_waves() -> None:
  ...
  page_tasks = [task for task in plan["tasks"] if task.get("kind") == "greenfield_page"]
  assert len(page_tasks) >= 3
  page_paths = {path for task in page_tasks for path in task.get("paths") or []}
  ...
  app_shell = next(task for task in plan["tasks"] if task["id"] == "greenfield-app-shell")
  assert app_shell["paths"] == ["src/App.jsx"]
  assert len(app_shell["depends_on"]) >= 2
  assert plan["coordination_contract"]["website_type"] == "ccaas"
  assert plan["coordination_contract"]["worker_protocol"] == "worktual-parallel-a2a-v1"
...
def test_greenfield_crm_plan_assigns_file_contracts_for_co_workers() -> None:
  ...
  assert contract["main_coding_agent"].startswith("Owns integration contract")
  task_contracts = {item["task_id"]: item for item in contract["task_contracts"]}
  assert task_contracts["greenfield-app-shell"]["depends_on"]
  assert task_contracts["greenfield-page-src-pages-leads-jsx"]["export_name"] == "Leads"
