...
from backend.agents.memory.context import (
  build_agent_flow_memory_block,
...
from backend.storage.permissions import require_memory_scope
from backend.storage.memory_framework import MemoryFrameworkStoreMixin
...
def test_session_context_rejects_topic_from_another_project() -> None:
  class _WrongTopicStore:
    def get_memory_chat_topic(self, user, *, chat_topic_id):
      return {
        ...
        "user_id": user.id,
        "rolling_summary": "Leaked context",
      }

    def get_memory_chat_session_state(self, *args, **kwargs):
      raise AssertionError("mismatched topic must not fall back to session memory")

  user = UserContext(id="user-1", email="u@example.com", role="editor")
  block = build_session_memory_context_block(
    _WrongTopicStore(),
    user,
...
def test_memory_scope_rejects_session_from_another_project() -> None:
  class _Cursor:
  ...
  class _Store:
    def get_project(self, project_id, user):
      return {"id": project_id, "owner_user_id": user.id}
  ...
  with pytest.raises(StorageError, match="does not belong"):
    require_memory_scope(
      ...
      user,
      project_id="project-1",
      chat_session_id="session-from-project-2",
      write=True,
    )
...
def test_persist_generation_memory_checkpoint_uses_atomic_store_path(monkeypatch):
  monkeypatch.setenv("ENABLE_EPISODIC_VECTOR_SEARCH", "false")

  class Store:
    atomic_payload: dict | None = None
    completed = False
    ...
    def find_memory_episode_by_run_id(self, user, **kwargs):
      return None

    def persist_generation_memory_checkpoint_atomic(self, user, **kwargs):
      self.atomic_payload = kwargs
      self.completed = True
      return {
        "snapshot": {"id": "snapshot-atomic"},
        "session_state": {"chat_session_id": kwargs["chat_session_id"], "update_count": 1},
        "episode": {"id": "episode-atomic", **(kwargs["episode_payload"] or {})},
      }

    def insert_memory_session_snapshot(self, *_args, **_kwargs):
      raise AssertionError("atomic store must not use separate snapshot write")

    def upsert_memory_chat_session_state(self, *_args, **_kwargs):
      raise AssertionError("atomic store must not use separate session state write")

    def insert_memory_episode(self, *_args, **_kwargs):
      raise AssertionError("atomic store must not use separate episode write")

    def complete_memory_checkpoint(self, *_args, **_kwargs):
      raise AssertionError("atomic store already completes checkpoint in transaction")

  store = Store()
  result = persist_generation_memory_checkpoint(
    store,
    UserContext(id="user-1", email="u@example.com", role="user"),
    project_id="project-1",
    chat_session_id="session-1",
    generation_run_id="run-1",
    prompt="Update checkout page copy",
    intent="website_update",
    outcome="completed",
    changed_paths=["src/pages/Checkout.jsx"],
  )

  assert result["status"] == "stored"
  assert result["snapshot_id"] == "snapshot-atomic"
  assert result["episode_id"] == "episode-atomic"
  assert result["episode_status"] == "stored"
  assert result["atomic_checkpoint"] is True
  assert store.completed is True
  assert store.atomic_payload["generation_run_id"] == "run-1"
  assert store.atomic_payload["episode_payload"]["scope"] == "personal"
...
def test_agent_flow_memory_block_merges_unified_and_chat_continuity():
  ...
  block = build_agent_flow_memory_block(
    ...
    user,
    project_id="project-1",
    prompt="also fix hero title color",
    chat_session_id="session-1",
...
def test_agent_flow_memory_block_skips_raw_chat_for_direct_update():
  store = _MemoryFrameworkStore()
  user = UserContext(id="user-1", email="u@example.com", role="user")
  store.session_states["session-1"] = {
    "chat_session_id": "session-1",
    "update_count": 3,
    "rolling_summary": "Old unrelated testimonialData import error",
  }
  store.episodes.append(
    {
      "id": "episode-old-error",
      "project_id": "project-1",
      "chat_session_id": "session-1",
      "scope": "chat_session",
      "searchable_summary": "Old user request: fix testimonialData import error",
      "outcome": "completed",
  ...
  messages = [
    ...
    {"role": "model", "content": "Repaired mockData exports."},
  ]
  block = build_agent_flow_memory_block(
    store,
    user,
  ...
  assert "CONVERSATION CONTINUITY" not in block
  assert "testimonialData" not in block
  assert "Relevant episodic memory" not in block


def test_real_memory_store_prunes_session_episodes_without_undefined_scope(monkeypatch):
  deleted_rows = [{"id": "episode-old"}]
  ...
  class Cursor:
    ...
    def execute(self, query, params):
      assert "delete from memory_episodes" in query.lower()
      assert params == ("user-1", "project-1", "session-1", 20)
  ...
  class Store(MemoryFrameworkStoreMixin):
    def connect(self):
      return Connection()

  monkeypatch.setattr(
    "backend.storage.memory_framework.require_memory_scope",
    lambda *_args, **kwargs: kwargs,
  )
  deleted = Store().prune_memory_episodes(
    UserContext(id="user-1", email="u@example.com", role="user"),
    project_id="project-1",
    chat_session_id="session-1",
  )
...
def test_real_memory_store_deletes_episode_with_project_write_scope(monkeypatch):
  class Cursor:
    def __enter__(self):
      return self
    ...
    def execute(self, query, params):
      assert "delete from memory_episodes" in query.lower()
      assert params == ("episode-1", "user-1", "project-1")
  ...
  store = Store()
  monkeypatch.setattr(
    "backend.storage.memory_framework.require_project",
    lambda *_args, **_kwargs: {"id": "project-1", "owner_user_id": "user-1"},
  )
  monkeypatch.setattr(
    "backend.storage.memory_framework.ensure_project_write",
...
def test_real_memory_store_lists_shared_project_episodes_across_users(monkeypatch):
  ...
  monkeypatch.setattr(
    "backend.storage.memory_framework.require_project",
    lambda *_args, **_kwargs: {"id": "project-1", "owner_user_id": "owner-1"},
  )
  monkeypatch.setattr(
    "backend.storage.memory_framework.ensure_project_read",
    lambda *_args, **_kwargs: None,
  )

  Store().list_memory_episodes(
    UserContext(id="member-1", email="member@example.com", role="user"),
    project_id="project-1",
    scope="shared",
  )

  assert "user_id = %s" not in str(observed["query"])
  assert observed["params"] == ("project-1", "shared", 12)


def test_real_memory_store_keeps_personal_episode_user_filter(monkeypatch):
  observed: dict[str, object] = {}
  ...
  assert "user_id = %s" in str(observed["query"])
  assert observed["params"] == ("member-1", "project-1", "personal", 12)


def test_real_memory_store_atomic_checkpoint_rolls_back_on_partial_failure(monkeypatch):
  class Cursor:
    def __init__(self):
      self.calls = 0
    ...
    def execute(self, _query, _params=None):
      self.calls += 1
      if self.calls == 2:
        raise RuntimeError("session state insert failed")
  ...
  with pytest.raises(RuntimeError, match="session state insert failed"):
    Store().persist_generation_memory_checkpoint_atomic(
      UserContext(id="user-1", email="u@example.com", role="user"),
      project_id="project-1",
      chat_session_id="session-1",
      generation_run_id="run-1",
      snapshot_kind="update_checkpoint",
      rolling_summary="Updated checkout copy",
    )
...
def test_checkpoint_failure_releases_processing_claim():
  class Store:
    ...
    def fail_memory_checkpoint(self, *, generation_run_id, error):
      self.failed = (generation_run_id, error)
      return True
  ...
  assert store.failed == ("run-1", "snapshot write failed")
