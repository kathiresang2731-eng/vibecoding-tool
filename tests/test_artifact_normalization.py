...
from backend.agents.orchestration.artifact_response import normalize_document_artifact, normalize_simple_code_artifact
...
def test_document_artifact_accepts_markdown_document_files():
  response = {
    "generated_website": {
      "files": [
        {
    ...
    "implementation_notes": {"self_checks": ["markdown document generated"]},
  }

  normalized = normalize_document_artifact(response)

  assert normalized["title"] == "Document Artifact"
  assert normalized["files"][0]["path"] == "docs/research-plan.md"
  assert normalized["files"][0]["code"].startswith("# Research Plan")


def test_document_artifact_converts_pdf_request_to_markdown_source():
  response = {
    "generated_website": {
      "files": [
        {
          "path": "proposal.pdf",
          "purpose": "PDF-ready proposal",
          "code": "# Proposal\n\nReady for export.\n",
        }
      ]
    }
  ...
  normalized = normalize_document_artifact(response, user_prompt="give me this as pdf")

  assert normalized["files"][0]["path"] == "proposal.pdf"
  assert normalized["files"][0]["code"].startswith("data:application/pdf;base64,")


def test_document_artifact_promotes_markdown_file_to_pdf_when_prompt_requests_pdf():
  response = {
    "generated_website": {
      "files": [
        {
          ...
          "code": "# APJ Abdul Kalam\n\nDetailed report.\n",
        }
      ]
    }
  }

  normalized = normalize_document_artifact(response, user_prompt="give me this detailed as pdf")

  assert normalized["files"][0]["path"] == "reports/history.pdf"
  assert normalized["files"][0]["code"].startswith("data:application/pdf;base64,")


def test_document_artifact_pdf_data_url_writes_as_real_pdf_bytes(tmp_path):
  from backend.local_workspace.io import write_local_project_files

  response = {
    "generated_website": {
      "files": [
        {
          "path": "reports/history.pdf",
          "purpose": "Detailed history",
          "code": "# APJ Abdul Kalam\n\nDetailed report.\n",
        }
      ]
  ...
  normalized = normalize_document_artifact(response, user_prompt="give me this detailed as pdf")
  write_local_project_files(tmp_path, normalized["files"])
  written = (tmp_path / "reports" / "history.pdf").read_bytes()

  assert written.startswith(b"%PDF-1.4")


def test_document_artifact_moves_website_paths_to_docs_surface():
  response = {
    "generated_website": {
      "files": [
        {
          "path": "src/pages/report.md",
          "purpose": "Report",
          "code": "# Report\n",
        }
      ]
  ...
  assert normalized["files"][0]["path"] == "docs/report.md"
