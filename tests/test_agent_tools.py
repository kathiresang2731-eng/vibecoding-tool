...
def test_preview_visual_qa_tool_returns_structured_interaction_failure(monkeypatch):
  def fake_browser_qa(**kwargs):
    return {
      "project_id": kwargs["project_id"],
      "status": "failed",
      ...
      "checks": [{"name": "requested_control_interaction", "status": "failed", "detail": "blank page"}],
      "warnings": ["The page became blank after clicking Launch Operation Hub."],
      "interaction": {
        "selected_control": {"label": "Launch Operation Hub", "tag": "button"},
        "current_url": "http://localhost/operations",
        "console_errors": ["ReferenceError: Operations is not defined"],
        "stack_traces": ["handleLaunch (src/App.jsx:20:5)"],
        "failed_requests": [],
        "blank_page": True,
      },
    }
  ...
  assert result["mode"] == "interaction_qa_failed"
  assert result["interaction"]["current_url"].endswith("/operations")
  assert result["interaction"]["console_errors"]


def test_preview_visual_qa_tool_blocks_interaction_commit_when_browser_is_unavailable(monkeypatch):
  monkeypatch.setattr(
    "backend.agent_tools.run_browser_preview_qa",
    lambda **kwargs: {
      "project_id": kwargs["project_id"],
      "status": "skipped",
      ...
      "browser_rendered": False,
      "checks": [],
      "warnings": ["No browser command was found."],
    },
  )
  ...
  result = execute_website_tool(
    ...
    type("User", (), {"id": "user-1"})(),
    {
      ...
      "preview_url": "/api/previews/project-1/v1/",
      "build_log": "built in 800ms",
      "interaction_prompt": "fix the Launch Operation Hub button",
    },
  )

  assert result["status"] == "failed"
  assert result["browser"]["failure_kind"] == "interaction_qa_unavailable"
...
def test_project_file_path_normalizer_allows_safe_tree_and_rejects_private_files():
  ...
  assert normalize_project_file_path("Crm-Project-worktual/index.html") == "index.html"
  assert normalize_project_file_path("Crm-Project-worktual/package.json") == "package.json"
  assert normalize_project_file_path("Crm-Project-worktual/src/App.jsx") == "src/App.jsx"
  ...
  with pytest.raises(LocalWorkspaceError, match="inside an ignored folder"):
    normalize_project_file_path("node_modules/package.json")
  with pytest.raises(LocalWorkspaceError, match="inside an ignored folder"):
    normalize_project_file_path("dist/index.html")
  ...
  with pytest.raises(LocalWorkspaceError, match="not allowed"):
    normalize_project_file_path("/Crm-Project-worktual/index.html")
