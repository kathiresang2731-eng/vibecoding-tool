...
def test_background_color_update_uses_scoped_model_patch_not_static_targeted_patch():
  ...
  assert analysis["execution_strategy"] == "scoped_model_patch"
...
def test_scoped_update_falls_back_for_navigation_interaction_no_patch():
  class InteractionRepairControlProvider(FakeControlProvider):
    def generate_json(self, prompt, **kwargs):
      if kwargs.get("trace_label") == "update_analysis_agent":
        return {
          "update_mode": "bug_fix",
          ...
          "execution_strategy": "scoped_model_patch",
          "scope": "small",
          "summary": "Wire the dashboard onboarding walkthrough button to the existing onboarding route.",
          "target_symbols": ["Start Onboarding Walkthrough", "Onboarding", "useNavigate"],
          "candidate_files": [
            "src/pages/Dashboard.jsx",
            "src/App.jsx",
            "src/pages/Onboarding.jsx",
            "src/worktual-router-shim.jsx",
          ],
          "target_files": ["src/pages/Dashboard.jsx"],
          "reference_files": ["src/App.jsx", "src/pages/Onboarding.jsx", "src/worktual-router-shim.jsx"],
          "interaction_summary": "Start Onboarding Walkthrough button (click): navigate to onboarding page",
          "interaction": {
          ...
          "required_agents": ["debug_patch_agent"],
          "preserve_rules": ["Preserve all unrelated routes, pages, styling, and content."],
          "allow_full_regeneration": False,
          "clarification_question": "",
          "reason": "The request identifies a concrete broken UI interaction and an existing target page.",
        }
      return super().generate_json(prompt, **kwargs)

  class EmptyInteractionPatchProvider(FakeArtifactProvider):
    def __init__(self):
      super().__init__()
      self.prompts = []
  ...
  artifact_provider = EmptyInteractionPatchProvider()
  written_files = []
  dashboard_code = (
    "import React from 'react';\n"
    ...
    "  return (\n"
    "    <main>\n"
    "      <button type=\"button\" className=\"primary\">Start Onboarding Walkthrough</button>\n"
    "    </main>\n"
    "  );\n"
    "}\n"
  )
  existing_files = [
    {"path": "package.json", "content": '{"scripts":{"build":"vite"},"dependencies":{"vite":"latest","react":"latest","react-dom":"latest","react-router-dom":"latest"}}'},
    {"path": "index.html", "content": '<title>Dashboard</title><div id="root"></div><script type="module" src="/src/main.jsx"></script>'},
    {"path": "src/main.jsx", "content": 'import React from "react";\nimport { createRoot } from "react-dom/client";\nimport App from "./App.jsx";\ncreateRoot(document.getElementById("root")).render(<App />);'},
    {"path": "src/index.css", "content": "body { margin: 0; }"},
    {
      ...
      "content": (
        ...
        "export default function App() {\n"
        "  return <HashRouter><Routes><Route path=\"/\" element={<Dashboard />} /><Route path=\"/onboarding\" element={<Onboarding />} /></Routes></HashRouter>;\n"
        "}\n"
      ),
    },
    {"path": "src/worktual-router-shim.jsx", "content": "export { HashRouter, Routes, Route, useNavigate } from 'react-router-dom';\n"},
    {"path": "src/pages/Dashboard.jsx", "content": dashboard_code},
    {"path": "src/pages/Onboarding.jsx", "content": "export default function Onboarding() { return <main>Onboarding</main>; }\n"},
  ]

  from backend.agents.agent_runtime.scoped_update.runtime_parts.sequence import run_scoped_update_sequence

  update_analysis = InteractionRepairControlProvider().generate_json("", trace_label="update_analysis_agent")
  update_analysis["enrichment_profile"] = "interaction_wiring"
  update_analysis["scoped_update_tasks"] = [
    {
      "id": "interaction-repair",
      "summary": update_analysis["summary"],
      "candidate_files": update_analysis["candidate_files"],
      "paths": update_analysis["candidate_files"],
      "group_paths": True,
    }
  ]
  ...
  result, changed_files, _task_results = run_scoped_update_sequence(
    artifact_provider,
    prompt="while click the Start Onboarding Walkthrough button failed to redirect to the onboarding page so fix that issue",
    update_analysis=update_analysis,
    existing_files=existing_files,
    code_search_matches=[],
  )

  assert result["changed_files"][0]["path"] == "src/pages/Dashboard.jsx"
  assert [item["path"] for item in changed_files] == ["src/pages/Dashboard.jsx"]
  if artifact_provider.prompts:
    assert "Interaction Contract" in artifact_provider.prompts[0] or "interaction" in artifact_provider.prompts[0]
  assert result["deterministic_fallback"] == "navigation_interaction_wiring"
  written_dashboard = changed_files[0]["content"]
  assert "useNavigate" in written_dashboard
  assert "onClick={() => navigate('/onboarding')}" in written_dashboard
...
def test_scoped_update_sequence_rejects_zero_aggregate_changed_files(monkeypatch):
  from backend.agents.agent_runtime.scoped_update.runtime_parts import sequence as sequence_module

  def fake_task_with_expansion(*_args, **kwargs):
    return (
      {
        "status": "completed",
        "summary": "No file edits were produced.",
        "edits": [],
        "changed_files": [],
        "requested_files": [],
        "clarification_question": "",
      ...
      [],
      kwargs["update_analysis"],
      [],
    )

  monkeypatch.setattr(sequence_module, "run_scoped_update_task_with_expansion", fake_task_with_expansion)

  with pytest.raises(ScopedUpdateGuardError, match="zero changed files"):
    sequence_module.run_scoped_update_sequence(
      object(),
      prompt="Change the deals page theme to red and black.",
      update_analysis={
        "update_mode": "feature_patch",
        "request_kind": "theme_color_update",
        "candidate_files": ["src/index.css", "src/pages/Deals.jsx"],
        "candidate_new_files": [],
        "scoped_update_tasks": [
          {"id": "style-css", "candidate_files": ["src/index.css"]},
          {"id": "style-deals-page", "candidate_files": ["src/pages/Deals.jsx"]},
        ],
      },
      existing_files=[
        {"path": "src/index.css", "content": "body { background: white; }\n"},
        {"path": "src/pages/Deals.jsx", "content": "export default function Deals(){ return <main>Deals</main>; }\n"},
      ],
      code_search_matches=[],
    )


def test_handle_scoped_update_agent_rejects_empty_sequence_before_staging(monkeypatch):
  from backend.agents.agent_runtime.actions.context import RuntimeActionContext
  from backend.agents.agent_runtime.actions.generation_parts import scoped_update as scoped_update_module

  def fake_sequence(*_args, **_kwargs):
    return (
      {"status": "completed", "summary": "No edits.", "changed_files": []},
      [],
      [{"id": "style-css", "changed_file_paths": []}],
    )

  def fail_if_staged(*_args, **_kwargs):
    raise AssertionError("empty scoped update should fail before patch staging")

  events: list[tuple[str, str, str | None]] = []
  monkeypatch.setattr(scoped_update_module, "run_scoped_update_sequence", fake_sequence)
  monkeypatch.setattr(scoped_update_module, "stage_candidate_patches_via_apply_patch", fail_if_staged)
  ...
  ctx = RuntimeActionContext(
    action="RUN_SCOPED_UPDATE_AGENT",
    state=state,
    decision={"next_agent": "Scoped Update Agent"},
    ...
    timeout_seconds=60,
    progress=lambda step, message, **kwargs: events.append((step, message, kwargs.get("status"))),
    runtime_objects={},
  )

  with pytest.raises(ScopedUpdateGuardError, match="zero changed files"):
    scoped_update_module.handle_scoped_update_agent(ctx)

  assert any(step == "agent.loop.scoped_update.no_changes" and status == "failed" for step, _message, status in events)
  assert "generated_website" not in state
